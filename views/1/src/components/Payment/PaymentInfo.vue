  ...
  <a-col :lg="8" :span="24">
    <a-flex class="payment__right-side" vertical>
      <a-spin
        ...
        size="large"
        wrapper-class-name="payment-info__preloader"
      >
        <a-flex justify="space-between">
          <a-typography-title :level="3" class="text-primary mb-0">{{
            appName
          }}</a-typography-title>
          <BackButton v-if="!screens.lg && !screens.xl" />
        </a-flex>
        <a-divider class="payment__divider text-primary">{{
          $t('payment.paymentInfo')
        }}</a-divider>
        <a-flex>
          <ul class="payment__list">
            <li class="payment__list-item">
              <a-typography-text class="text-primary fw-bold">{{
                $t('payment.domain')
              }}</a-typography-text>
            ...
            <li class="payment__list-item">
              <a-typography-text class="text-primary text-nowrap fw-bold">{{
                $t('payment.paymentNumber')
              }}</a-typography-text>
              <a-tooltip overlay-class-name="payment__list-item-tooltip">
                <template #title
                  ><a-typography-text class="text-primary text-nowrap">{{
                    payment?.id
                  }}</a-typography-text></template
                >
                <a-typography-text class="text-primary text-nowrap-ellipsis">{{
                  payment?.id
                }}</a-typography-text>
              </a-tooltip>
            </li>
            <li class="payment__list-item">
              <a-typography-text class="text-primary fw-bold text-nowrap">{{
                $t('payment.totalSend')
              }}</a-typography-text>
              <a-tooltip overlay-class-name="payment__list-item-tooltip">
                <template #title
                  ><a-typography-text class="text-primary text-nowrap"
                    >{{ payment?.fullAmount }} {{ payment?.currencyFrom }} (${{
                      payment?.fullAmountInCurrency
                    }})</a-typography-text
                  ></template
                >
                <a-typography-text class="text-primary text-nowrap-ellipsis"
            ...
            <li class="payment__list-item">
              ...
              <a-typography-text class="text-primary"
                >{{ commission }}%</a-typography-text
              >
            </li>
            <li class="payment__list-item">
              <a-typography-text class="text-primary fw-bold">{{
                $t('payment.invoiceExpiresIn')
              }}</a-typography-text>
              <a-typography-text class="text-primary">{{
                time
              }}</a-typography-text>
            ...
            <li class="payment__list-item">
              ...
              <a-typography-text :class="statusColor">{{
        ...
        <a-flex vertical>
          <a-typography-text class="text-primary fw-bold payment__to-pay-from"
            >{{ payment?.fullAmount }}
            {{ payment?.currencyFrom }}</a-typography-text
          >
          <a-typography-text class="fw-bold payment__to-pay-to"
            >{{ payment?.coinToFullQuantity }}
            {{ payment?.currencyTo }}</a-typography-text
          >
        </a-flex>
      </a-spin>
    </a-flex>
...
import { wackyRound } from '../../heplers/numbers';
...
export default {
  ...
  setup() {
    ...
    const calculateRemainingTime = async () => {
      ...
      if (
        milDiff <= 0 ||
        payment.value.status === PAYMENT_STATUSES.SUCCESSFUL
      ) {
        clearInterval(timer);
      ...
      time.value = moment(
        moment(payment.value.expirationDate).diff(moment()),
      ).format('mm:ss');
